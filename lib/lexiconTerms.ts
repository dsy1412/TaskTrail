...
function applyLexiconEnrichment(entry: LexiconEntry, enrichment: LexiconEnrichment): LexiconEntry {
  const preferProfessional = Boolean(enrichment.preferProfessional);
  return {
    ...entry,
    ipa: filledText(entry.ipa, enrichment.ipa, preferProfessional),
    phonics: filledText(entry.phonics, enrichment.phonics, preferProfessional),
    fieldContext: filledText(entry.fieldContext, enrichment.fieldContext, preferProfessional),
    meaning: filledText(entry.meaning, enrichment.meaning, preferProfessional),
    association: filledText(entry.association, enrichment.association, preferProfessional),
    example: filledText(entry.example, enrichment.example, preferProfessional),
    exampleTranslation: filledText(entry.exampleTranslation, enrichment.exampleTranslation, preferProfessional),
    related: entry.related.length ? entry.related : enrichment.related ?? [],
  };
}

function filledText(current?: string, fallback?: string, preferProfessional = false) {
  if (!fallback) return current?.trim() ? current : "";
  if (!current?.trim()) return fallback;
  if (preferProfessional && !containsChinese(current) && containsChinese(fallback)) return fallback;
  return current;
}
