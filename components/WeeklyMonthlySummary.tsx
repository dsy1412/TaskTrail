  ...
  return (
    <aside className="glass-panel rounded-[2rem] p-5">
      ...
      <div className="space-y-4">
        <section>
          <h3 className="text-sm font-semibold text-slate-50">Goal cards</h3>
          <div className="mt-2 grid gap-2">
            {[...weeklyGoals, ...monthlyGoals].slice(0, 5).map((goal) => (
              <div key={goal.id} className="rounded-lg border border-slate-800 bg-slate-900 p-3 shadow-sm">
                <p className="text-sm font-semibold text-slate-50">{goal.title}</p>
                <p className="mt-1 text-xs font-medium text-slate-400">{goal.module}</p>
              </div>
            ))}
            {!weeklyGoals.length && !monthlyGoals.length ? (
              <p className="rounded-lg border border-slate-800 bg-slate-900 p-3 text-sm font-medium text-slate-300">
